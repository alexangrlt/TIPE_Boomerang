...
def simulate_projectile(position_init, vitesse_init, config, dt=0.0005, t_max=15):
    ...
    # Inclinaison autour de Y : 80 deg par rapport à la verticale
    # Le boomerang est lancé en +X, donc l'inclinaison doit être autour de Y
    # (rotation autour de Y -> la face du boomerang regarde vers le haut-gauche)
    rot = R.from_euler('y', -80.0, degrees=True)

    I     = config.matrice_inertie()
    I_inv = np.linalg.inv(I)

    # omega initial : ~1200 rpm = 125.7 rad/s autour de la normale du plan
    # La normale au plan dans le repère corps est [0, 0, 1]
    # On applique la rotation initiale pour l'exprimer dans le repère monde
...
def compute_forces_be(elements, v_cm, omega_monde, rot, config):
    ...
    for e in elements:
        ...
        # alpha = angle entre v_rel projete et la normale au profil
        # On projette v_rel dans le plan perpendiculaire a axe_pale
        # puis on decompose en composante selon n_plan et selon v_t
        # -------------------------------------------------------
        ...
        # Composante normale au plan (portance) et tangentielle (trainee)
        ...
        # La portance est perpendiculaire a v_rel ET a l'axe de pale
        # Convention : cross(axe_pale, v_rel) donne la direction
        # de la portance pour un profil en incidence positive
        # -------------------------------------------------------
